...
from formshare.processes.db import get_project_id_from_name, get_project_groups, add_group, get_group_data, \
    ...
    def process_view(self):
        ...
        if self.request.method == 'POST':
            ...
            if group_data['group_desc'] != "":
                next_page = self.request.params.get('next') or self.request.route_url('groups',
                                                                                      userid=user_id,
                                                                                      projcode=project_code)
                added, message = add_group(self.request, project_id, group_data)
                if added:
                    self.request.session.flash(self._('The group was added to this project'))
            ...
            group_data = {}
        return {'groupData': group_data, 'projectDetails': project_details, 'userid': user_id}
    ...
    def process_view(self):
        ...
        if self.request.method == 'POST':
            ...
            next_page = self.request.params.get('next') or self.request.route_url('groups',
                                                                                  ...
                                                                                  projcode=project_code)
            if group_data['group_desc'] != "":
                edited, message = modify_group(self.request, project_id, group_id, group_data)
                if edited:
                    self.request.session.flash(self._('The group was edited successfully'))
                    self.returnRawViewResult = True
                    return HTTPFound(next_page)
                else:
                    self.errors.append(message)
    ...
    def process_view(self):
        ...
        if self.request.method == 'POST':
            ...
            if removed:
                ...
                self.request.session.flash(self._('Unable to delete the group: ') + message)
                return HTTPFound(next_page)
...
class GroupMembersView(PrivateView):
    ...
    def process_view(self):
        ...
        if self.request.method == 'POST':
            ...
            next_page = self.request.params.get('next') or self.request.route_url('group_members',
                                                                                  userid=user_id,
                                                                                  projcode=project_code,
                                                                                  groupid=group_id)
...
class RemoveMember(PrivateView):
    ...
    def process_view(self):
        ...
        if self.request.method == 'POST':
            ...
            removed, message = remove_assistant_from_group(self.request, project_id, group_id, project_id2, member_id)
            next_page = self.request.params.get('next') or self.request.route_url('group_members',
                                                                                  userid=user_id,
                                                                                  projcode=project_code,
                                                                                  groupid=group_id)
