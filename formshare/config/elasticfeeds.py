...
def configure_manager(settings):
    ...
    try:
        number_of_shards_in_feeds = int(settings['elasticfeeds.number_of_shards_in_feeds'])
    except KeyError:
        number_of_shards_in_feeds = 5

    try:
        number_of_replicas_in_feeds = int(settings['elasticfeeds.number_of_replicas_in_feeds'])
    except KeyError:
        number_of_replicas_in_feeds = 1

    try:
        number_of_shards_in_network = int(settings['elasticfeeds.number_of_shards_in_network'])
    except KeyError:
        number_of_shards_in_network = 5

    try:
        number_of_replicas_in_network = int(settings['elasticfeeds.number_of_replicas_in_network'])
    except KeyError:
