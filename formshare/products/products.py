...
def get_form_products(request, project, form):
        ...
        for a_product in used_products:
            ...
            if product_info is not None:
                if product_info["plugin"]:
                    plugin_description = plugin.get_product_description(
                        request, a_product["code"]
                    )
    ...
    for a_product in result:
        a_product["outputs"] = get_form_outputs(
            request, project, form, a_product["code"]
        )
...
def get_product_directory(request, project, form, product):
    try:
        if product_found(product):
            path = os.path.join(
                request.registry.settings["repository.path"],
                *["products", project, form, product]
            )
        ...
        log.error(
            "Unable to create product directory for product {} in project {} form {}. Error: {}".format(
                product, project, form, str(e)
            )
        )
        return None
...
def add_metadata_to_product(product, key, value):
    try:
        ...
        log.error(
            "Unable to add metadata to product {}. Key: {}. Value {}. Error: {}".format(
                product, key, value, str(e)
            )
        )
