import os
import uuid

from formshare.processes.db.form import (
    get_form_xml_create_file,
)
from formshare.products import register_product_instance
from formshare.products.export.zip_json.celery_task import build_zip_json
from formshare.processes.db.products import product_max_number
import formshare.plugins as p


def generate_public_zip_json_file(
    request,
    user,
    project,
    form,
    odk_dir,
    form_schema,
    options=1,
    include_multiselect=False,
    include_lookups=False,
):
    if product_max_number(request, project, form, "zip_json_public_export"):
        return False
    settings = {}
    for key, value in request.registry.settings.items():
        if isinstance(value, str):
            settings[key] = value

    uid = str(uuid.uuid4())
    paths = ["products", uid + ".zip"]
    repo_dir = request.registry.settings["repository.path"]
    zip_file = os.path.join(repo_dir, *paths)

    create_xml_file = get_form_xml_create_file(request, project, form)

    task = None
    for plugin in p.PluginImplementations(p.IExportGenerator):
        task = plugin.json_export(
            settings,
            odk_dir,
            form_schema,
            form,
            create_xml_file,
            request.registry.settings["auth.opaque"],
            zip_file,
            True,
            request.locale_name,
            options,
            include_multiselect,
            include_lookups,
        )
        if task is not None:
            break

    if task is None:
        task = build_zip_json.apply_async(
            (
                settings,
                odk_dir,
                form_schema,
                form,
                create_xml_file,
                request.registry.settings["auth.opaque"],
                zip_file,
                True,
                request.locale_name,
                options,
                include_multiselect,
                include_lookups,
            ),
            queue="FormShare",
        )
    register_product_instance(
        request,
        user,
        project,
        form,
        "zip_json_public_export",
        task.id,
        zip_file,
        "application/zip",
        False,
        True,
    )
    return True


def generate_private_zip_json_file(
    request,
    user,
    project,
    form,
    odk_dir,
    form_schema,
    options=1,
    include_multiselect=False,
    include_lookups=False,
):
    if product_max_number(request, project, form, "zip_json_private_export"):
        return False
    settings = {}
    for key, value in request.registry.settings.items():
        if isinstance(value, str):
            settings[key] = value

    uid = str(uuid.uuid4())
    paths = ["products", uid + ".zip"]
    repo_dir = request.registry.settings["repository.path"]
    zip_file = os.path.join(repo_dir, *paths)

    create_xml_file = get_form_xml_create_file(request, project, form)

    task = None
    for plugin in p.PluginImplementations(p.IExportGenerator):
        task = plugin.json_export(
            settings,
            odk_dir,
            form_schema,
            form,
            create_xml_file,
            request.registry.settings["auth.opaque"],
            zip_file,
            False,
            request.locale_name,
            options,
            include_multiselect,
            include_lookups,
        )
        if task is not None:
            break

    if task is None:
        task = build_zip_json.apply_async(
            (
                settings,
                odk_dir,
                form_schema,
                form,
                create_xml_file,
                request.registry.settings["auth.opaque"],
                zip_file,
                False,
                request.locale_name,
                options,
                include_multiselect,
                include_lookups,
            ),
            queue="FormShare",
        )

    register_product_instance(
        request,
        user,
        project,
        form,
        "zip_json_private_export",
        task.id,
        zip_file,
        "application/zip",
        False,
        False,
    )
    return True
