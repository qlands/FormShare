...
def upgrade():
    ...
    for a_form in forms:
        try:
            sql = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{}'".format(a_form.form_schema)
            tables = conn.execute(sql).fetchall()
            for a_table in tables:
                table_name = a_table[0]
                if not table_name.startswith('lkp') and not table_name.startswith('audit'):
                    sql = "ALTER TABLE {}.{} ADD COLUMN root_rowuuid varchar(80)".format(a_form.form_schema, table_name)
                    conn.execute(sql)
