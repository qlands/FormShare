...
def update_form_separation_file(request, project, form, separation_file):
    ...
    request.dbsession.query(Form).filter(Form.project_id == project).filter(
        Form.form_id == form
    ).update({"form_sepfile": separation_file})
    ...
    for table in xml_tables:
        ...
        new_table = Septable(
            project_id=project, form_id=form, table_name=table_name, table_desc=""
        )
        ...
        for group in xml_groups:
            ...
            new_group = Sepsection(
                project_id=project,
                form_id=form,
                table_name=table_name,
                section_id=section_id,
                section_name=group.get("name"),
                section_desc=group.get("description"),
            ...
            for item in xml_items:
                ...
                new_item = Sepitem(
                    ...
                    item_xmlcode=item.get("xmlcode"),
                    item_notdisplay=not_display,
                    item_order=item_order,
                    section_project=project,
                    section_form=form,
                    section_table=table_name,
                    section_id=section_id,
                )
...
def table_belongs_to_form(request, project, form, table_name):
    data = (
        request.dbsession.query(Septable)
        .filter(Septable.project_id == project)
        .filter(Septable.form_id == form)
        .filter(Septable.table_name == table_name)
        .first()
    )
    if data is None:
        return False
...
def get_table_items(request, project, form, table_name, just_main=False):
    ...
    sql = (
        "SELECT sepsection.section_id,sepsection.section_name,sepsection.section_desc,"
...
def is_separation_ok(request, project, form, table_name):
    items = (
        request.dbsession.query(
            Sepitem.section_id, label("totitems", func.count(Sepitem.section_id))
        )
        .filter(Sepitem.project_id == project)
        .filter(Sepitem.form_id == form)
        .filter(Sepitem.table_name == table_name)
        .group_by(Sepitem.section_id)
        .all()
    )
...
def save_separation_order(request, project, form, table_name, order, order2):
    ...
    request.dbsession.query(Sepitem).filter(Sepitem.project_id == project).filter(
        Sepitem.form_id == form
    ).filter(Sepitem.table_name == table_name).delete()
    ...
    for item in order:
        if item["type"] == "group":
            pos = pos + 1
            request.dbsession.query(Sepsection).filter(
                Sepsection.project_id == project
            ).filter(Sepsection.form_id == form).filter(
                Sepsection.table_name == table_name
            ).filter(
                Sepsection.section_id == item["id"].replace("GRP", "")
            ).update(
                {"section_order": pos}
            )
            ...
            if "children" in item.keys():
                for child in item["children"]:
                    ...
                    new_item = Sepitem(
                        ...
                        form_id=form,
                        table_name=table_name,
                        item_name=child["id"].replace("QST", ""),
                        item_desc=child["desc"],
                        item_notdisplay=child["display"],
                        section_project=project,
                        section_form=form,
                        section_table=table_name,
...
def get_group_number_of_items(request, project, form, table_name, section_id):
    res = (
        request.dbsession.query(Sepitem)
        .filter(Sepitem.project_id == project)
        .filter(Sepitem.form_id == form)
        .filter(Sepitem.table_name == table_name)
        .filter(Sepitem.section_id == section_id)
        .count()
    )
    return res
...
def get_group_name_from_id(request, project, form, table_name, section_id):
    res = (
        request.dbsession.query(Sepsection)
        .filter(Sepsection.project_id == project)
        .filter(Sepsection.form_id == form)
        .filter(Sepsection.table_name == table_name)
        .filter(Sepsection.section_id == section_id)
        .first()
    )
    if res is not None:
...
def generate_separation_file(request, project, form):
    ...
    for aTable in tables:
        ...
        groups = (
            ...
            .filter(Sepsection.project_id == project)
            .filter(Sepsection.form_id == form)
            .filter(Sepsection.table_name == aTable.table_name)
            .all()
        )
        for aGroup in groups:
            e_group = etree.SubElement(
                e_table,
                "group",
                name=aGroup.section_name,
                description=aGroup.section_desc,
            )
            items = (
                request.dbsession.query(Sepitem)
                .filter(Sepitem.section_project == project)
                .filter(Sepitem.section_form == form)
                .filter(Sepitem.section_table == aTable.table_name)
                .filter(Sepitem.section_id == aGroup.section_id)
                .all()
            )
            for item in items:
...
def get_tables_to_separate(request, project, form):
    tables = (
        request.dbsession.query(
            Sepitem.section_table,
            Sepitem.section_id,
            label("totitems", func.count(Sepitem.item_name)),
        )
        .filter(Sepitem.project_id == project)
        .filter(Sepitem.form_id == form)
        .group_by(Sepitem.section_table, Sepitem.section_id)
        .all()
    )
...
def delete_group(request, project, form, table_name, section_id):
    try:
        request.dbsession.query(Sepsection).filter(
            ...
            Sepsection.section_id == section_id
        ).delete()
        return True, ""
    except Exception as e:
        return False, str(e)
...
def add_group(request, project, form, table_name, section_name, section_desc):
    ...
    exist = (
        ...
        .filter(Sepsection.form_id == form)
        .filter(Sepsection.table_name == table_name)
        .filter(Sepsection.section_name == section_name)
        .first()
    )
    if exist is None:
        res = (
            request.dbsession.query(
                label("maxorder", func.max(Sepsection.section_order))
            )
            .filter(Sepsection.project_id == project)
            .filter(Sepsection.form_id == form)
            .filter(Sepsection.table_name == table_name)
