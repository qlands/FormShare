...
class ChangeMyAssistantPassword(AssistantView):
    def process_view(self):
        if self.request.method == 'POST':
            next_page = self.request.params.get('next') or self.request.route_url('assistant_forms',
                                                                                  userid=self.userID,
            ...
            if assistant_data['coll_password'] != '':
                if assistant_data['coll_password'] == assistant_data['coll_password2']:
                    project_of_assistant = get_project_from_assistant(self.request, self.userID, self.projectID,
                                                                      self.assistantID)
                    if check_assistant_login(project_of_assistant, self.assistantID, assistant_data['old_password'],
                                             self.request):

                        changed, message = change_assistant_password(self.request, project_of_assistant,
                                                                     self.assistantID, assistant_data['coll_password'])
                        if changed:
                            policy = self.get_policy('assistant')
                            headers = policy.forget(self.request)
                            next_page = self.request.route_url('assistant_login', userid=self.userID,
                                                               projcode=self.projectCode,
...
class GetQRCode(AssistantView):
    def process_view(self):
        ...
        url = self.request.route_url('project_details', userid=self.userID,
                                     projcode=self.projectCode)
        assistant_password = get_assistant_password(self.request, self.userID, self.projectID, self.assistantID)
        odk_settings = {'admin': {"change_server": True, "change_form_metadata": False},
                        'general': {"change_server": True, "navigation": "buttons",
                                    'server_url': url, 'username': self.assistantID, 'password': assistant_password.decode()}}
