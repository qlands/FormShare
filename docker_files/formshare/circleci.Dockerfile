...
RUN sudo wget https://dev.mysql.com/get/mysql-apt-config_0.8.36-1_all.deb
RUN sudo dpkg -i ./mysql-apt-config_0.8.36-1_all.deb
...
RUN sudo git clone https://github.com/qlands/odktools.git -b stable-2.18
