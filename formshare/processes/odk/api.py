...
def generate_manifest(media_file_array):
    ...
    Every key of a media file dict becomes a child element, except "type",
    which the OpenRosa spec puts on the mediaFile as an attribute when present.
    Nothing in FormShare sets it today; the handling stays because it is the
    spec's, not a feature's. Any other key stays a child, so <integrityUrl>
...
def refresh_published_lists(request, project_id, form, form_files):
    ...
    for a_file in form_files:
        ...
        # The regeneration decision is per stored file, not per registry list.
        # The list's list_lastgen is the edition marker (shared across every
        # form that serves the list); whether *this* form's copy needs
        # regenerating is answered by the media file's own file_lastgen. A
        # file_lastgen of None means this copy was never generated by us --
        # it is the placeholder or sample the user attached when the form was
        # uploaded -- so it is stale by definition and must be generated over,
        # or the manifest would serve the upload verbatim. Gating on
        # list_lastgen instead let a re-added form (or a second consumer of
...
def apply_registry_links(request, project, form, create_xml_file, create_file):
    """Wires a form's list consumers into its schema at repository build.

    Because a published list is generated *after* the form is built, the form
    is converted against a placeholder file, so RSTools cannot infer a
    select_one_from_file column's type and emits it as int -- which cannot even
    hold the rowuuid the device stores. So for every consumer this retypes the
    selector to the source's rowuuid and adds a foreign key to
    ``<source>.<table>(rowuuid)`` ON DELETE RESTRICT. The one consumer marked
    the case link additionally gets the membership trigger (via the maintable
    creator_* / block_trigger attributes RSTools reads), making the form a
    child of that table -- the equivalent of the classic case follow-up.

    All of it is expressed as create.xml attributes and realised by one
    createFromXML run, so a form with no consumers is left untouched.
    docs/formshare_case_management/formshare.md section 3.4.
    """
    sources = get_consumer_sources(request, project, form)
    if not sources:
        return 0, ""
    ...
    tree.write(
        create_xml_file, pretty_print=True, xml_declaration=True, encoding="utf-8"
    )
    create_from_xml = os.path.join(
        request.registry.settings["odktools.path"],
        *["utilities", "createFromXML", "createfromxml"]
    )
    args = [create_from_xml, "-i " + create_xml_file, "-o " + create_file]
    proc = Popen(args, stdout=PIPE, stderr=PIPE)
    stdout, stderr = proc.communicate()
    if proc.returncode != 0:
        log.error(
            "Registry links createFromXML error {}. Message: {}. Command: {}".format(
                proc.returncode,
        ...
        if p.returncode == 0:
            if not for_merging:
                ...
                if form_data["form_case"] == 1 and form_data["form_casetype"] > 1:
                    ...
                    # Which column of the creator a follow-up points at: the
                    # case identifier the project owner nominated. The native
                    # case-management registry will link on rowuuid instead
                    # (docs/formshare_case_management/), but that arrives with
                    # the registry, not before.
                    link_field = form_creator_data["form_pkey"]
                ...
                # A form that consumes a published list marked as the case link
                # becomes a child of that list's source table. This is the
                # native case-management equivalent of the classic block above
                # and is mutually exclusive with it: a registry follow-up is not
                # a classic case form, so form_case is 0 and the block above did
                # nothing. docs/formshare_case_management/formshare.md section 3.4.
                if not (form_data["form_case"] == 1 and form_data["form_casetype"] > 1):
                    # Reconcile the stored consumers with what the form now
                    # references, then wire them in. Sync here as well as in the
                    # UI so a build always reflects the current form even if the
                    # owner never opened the case-links page.
                    sync_form_consumers(
                        request,
                        project,
                        form,
                        get_fields_from_table_in_file(create_xml_file, "maintable"),
                    )
                    link_code, link_message = apply_registry_links(
                        request, project, form, create_xml_file, create_file
                    )
                    if link_code != 0:
                        return link_code, link_message
...
def set_rowuuid_from_meta(submission_data):
    """Adopts a device-minted rowuuid as the row's rowuuid.

    A client that mints the row identity on the device (FormShare Collect,
    through KotlinRosa) sends it as meta/rowuuid. Taking it as the rowuuid
    gives the row one identity instead of two: JSONToMySQL stores an explicit
    rowuuid, and the unique index on that column then makes a resend of the
    same submission collide rather than insert a second row.

    Does nothing when the key is absent or empty, which is every submission
    from a client that does not mint it -- those rows keep the server-minted
    default, exactly as before.
    """
    row_uuid = str(submission_data.get("meta/rowuuid", "") or "").strip()
    if row_uuid:
        submission_data["rowuuid"] = row_uuid
    ...
    if schema is not None:
        if schema != "":
            ...
            with open(temp_json_file, "r") as f:
                ...
                set_rowuuid_from_meta(submission_data)
                geopoint_variables = get_form_geopoints(request, project, form)
