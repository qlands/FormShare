...
def cancel_task(request, user, task_id):
    new_cancelled_task = FinishedTask(
        task_id=task_id,
        task_enumber=-2,
        task_error="Task canceled by {} on {}".format(
...
def task_exists(request, project, form, task):
    res = (
        ...
        .filter(Product.celery_taskid == task)
        .first()
    )
    if res is None:
        return False
...
def get_task_status(request, task_id):
    res = (
        request.dbsession.query(FinishedTask)
        .filter(FinishedTask.task_id == task_id)
        .first()
    )
    if res is not None:
